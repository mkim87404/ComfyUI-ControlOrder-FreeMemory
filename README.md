...
The memory clean up + linear passthrough of data facilitated by this node can help to **speed up certain workflows** involving multiple models (e.g. CLIP, VAE, Diffusion Model, etc.) that compete for the same VRAM/RAM but don't all need to be loaded at the same time, by separating each model's operation into its own segment and linearly chaining them together with multiple instances of this node, where only the necessary outputs of each segment (e.g. latents, conditioning, images, models, etc.) are passed on to the next segment while unloading all/some models that are no longer needed in the workflow.

This becomes especially effective in freeing VRAM/RAM between multiple KSamplers (e.g. Wan 2.2 High & Low KSamplers), or before & after VAE Encode / VAE Decode / Load CLIP / Load Model / etc. that allow more of the models' layers to be loaded onto the GPU VRAM and less offloaded to the slower System RAM, resulting in faster overall inference times and letting the user push to higher resolution media outputs before hitting OOM. (see [Limitations](#limitations) for more details)
...
- If your device already has enough VRAM & RAM to keep all required models loaded simultaneously, then enabling the optional "free_memory" operation on this node may produce counterproductive results, due to the unnecessary overhead introduced by repeatedly unloading and reloading the models. Similarly, you may only see performance gains at higher resolution media inferences that push towards your device's VRAM/RAM capacity and without tiled-VAE decoding, because with lower resolution inferences or tiled-VAE decoding, your VRAM can often fully load the VAE model without having to compete or wait for additional RAM offload space, meaning that you're already at the maximum inference speed for your device. Hence, it is recommended that you experiment with this node's "free_memory" toggle and switch it on/off depending on your device specifications, the resolution of media being generated, and which type of nodes / model workflows you are using (e.g. Wan 2.2 workflows tend to benefit more from this node's memory cleanup operation than LTX 2.3 workflows, possibly due to LTX 2.3 being a newer model architecture with more optimized VAE performances and related nodes that handle them, etc.) to find the fastest/optimal setting. When the "free_memory" toggle is off, any model unloading / memory management logic will be completely bypassed, and the node will continue to function correctly as a simple router with infinite I/Os that can be used to control the execution order of nodes. Please note that as ComfyUI and its model management logic is consistently evolving, any performance benefits offered by this node are subject to change at any time.
...
- Some GGUF / Quantized model loaders have custom unloading logic that may not always respect the keep_loaded feature of this node's "free_memory" operation, and may drop itself before entering the node much like how CLIP models behave. Likewise, for any models routed through this node that fail to persist or produce unexpected outcomes (e.g. CLIPTextEncode or KSamplers hanging indefinitely or throwing OutOfMemory errors, etc.), you can always fallback to using the models immediately after loading instead of routing them through this node, and load them back in on later chains whenever you need them again.
